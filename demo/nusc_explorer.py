...
def trans_to_kitti(annos, path):
    with open(path, 'wt') as f:
        for anno in annos:
            if 'pedestrian' in anno.name:
                label = 'Pedestrian'
            elif 'car' in anno.name:
                label = 'Car'
            elif 'truck' in anno.name:
                label = 'Truck'
            elif 'bicycle' in anno.name or 'motorcycle' in anno.name:
                label = 'Cyclist'
            else:
                continue
            
            # yaw = anno.orientation.radians + np.pi/2
            yaw, _, _ = anno.orientation.yaw_pitch_roll
            yaw = yaw + np.pi/2
            x = -anno.center[1]
            y = anno.center[0]
            z = anno.center[2]
            ...
            # if anno.orientation.radians > -1 and anno.orientation.radians < 0:
            kitti_label_str = '{} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {}'.format(label, 0, 0, 0,
...
def _test():
    ...
    # nusc_exp.render_sample_data(sample_data_token, nsweeps=10)
    # print('----------------------')
    ...
    for sample in tqdm(samples):
        sample_data_token = sample['data'][lidar_names[0]]
        pc, annos = nusc_exp.get_pc_with_anno(sample_data_token, nsweeps=4)
        annos_filtered = []
        for anno in annos:
            anno_meta = nusc.get('sample_annotation', anno.token)
            if anno_meta['num_lidar_pts'] > 10:
                annos_filtered.append(anno)

        data_root = '/data/privatedata/2022-02-18+nusc-mini/total/'
        label_file = os.path.join(data_root, 'label_2', sample_data_token+'.txt')
        pcd_file = os.path.join(data_root, 'velodyne', sample_data_token+'.bin')
        trans_to_kitti(annos_filtered, label_file)
        pc = np.pad(pc,(0,1)).transpose()
        pc[:,2] = pc[:,2] + NUSC_OFFSET_Z
        pc.tofile(pcd_file)
        i = i + 1
        if i > 3000:
            break
