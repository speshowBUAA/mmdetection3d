...
def simplify_onnx(onnx_model):
  ...
  input_new = gs.Variable(name="input", dtype=np.float32, shape=(MAX_VOXELS, 64, 10))
  ...
  first_node_first_pillarvfe = [node for node in graph.nodes if node.op == "MatMul"][0]
  first_node_last_pillarvfe = [node for node in graph.nodes if node.op == "MatMul"][1]
  next_node = current_node = first_node_last_pillarvfe
  ...
  inputs = [last_node_pillarvfe.outputs[0], X]
  ...
  graph.inputs = [first_node_first_pillarvfe.inputs[0] , X]
  graph.outputs = [tmap["box_preds"], tmap["cls_preds"], tmap["dir_cls_preds"]]
...
if __name__ == '__main__':
    ...
    # onnx.save(model_simp, "pointpillar_sim_middle.onnx")

    model_simp = simplify_onnx(model_simp)
    onnx.save(model_simp, "pointpillar_sim.onnx")
    # debug_onnx(onnx.load(model_file))
